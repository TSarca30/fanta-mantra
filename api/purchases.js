import { getPurchases, recordPurchase, removePurchase, TEAMS } from "./_sheets.js";

export default async function handler(req, res) {
  try {
    if (req.method === "GET") return res.status(200).json({ purchases: await getPurchases() });
    if (req.method === "DELETE") return res.status(200).json({ ok: await removePurchase(req.body?.playerId) });
